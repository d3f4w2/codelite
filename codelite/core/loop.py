...
class AgentLoop:
    ...
    def run_turn(self, session_id: str, user_input: str) -> str:
        ...
        tools = self.tool_router.tool_schemas()
        ...
        for step in range(1, self.config.runtime.max_steps + 1):
            ...
            if result.tool_calls:
                ...
                for call in result.tool_calls:
                    ...
                    try:
                        tool_result = self.tool_router.dispatch(call.name, call.arguments)
            ...
            answer = (result.text or "").strip() or "模型未返回文本结果。"
        ...
        raise RuntimeError(f"达到最大步数 {self.config.runtime.max_steps}，本轮停止。")
