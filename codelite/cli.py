...
_SHELL_LOCAL_COMMANDS: tuple[_ShellLocalCommand, ...] = (
    ...
    _ShellLocalCommand("version", "version", "Show current shell version", "显示当前版本"),
    _ShellLocalCommand("plan", "plan", "Switch to plan mode", "切换到规划模式", aliases=("planner",)),
    _ShellLocalCommand("act", "act", "Switch to act mode", "切换到行动模式", aliases=("accept",)),
    _ShellLocalCommand("mode", "mode [name]", "Show or set current mode", "查看或切换当前模式"),
    _ShellLocalCommand("status", "status", "Show runtime health summary", "查看运行时健康摘要", aliases=("health",)),
    _ShellLocalCommand("session", "session", "Show current session summary", "查看当前会话摘要", aliases=("sid", "session id")),
    _ShellLocalCommand("resume", "resume [id]", "Resume a saved chat", "恢复历史会话"),
    _ShellLocalCommand("rename", "rename <name>", "Rename the current thread", "重命名当前会话"),
    _ShellLocalCommand("replay", "replay [N]", "Replay current or latest N sessions", "回放会话历史"),
    _ShellLocalCommand("todo", "todo", "Show current todo snapshot", "查看当前 Todo 快照"),
    _ShellLocalCommand("tasks", "tasks", "Show task board", "查看任务看板"),
    _ShellLocalCommand("task", "task ...", "Task actions (show/claim/release/block/retry/jump)", "执行单任务操作"),
    _ShellLocalCommand("team", "team ...", "Run Agent Team demo (default), or /team board for board view", "运行或查看 Agent Team 流程"),
    _ShellLocalCommand("subagents", "subagents ...", "Alias of /team run <task>", "/team run 的快捷别名"),
    _ShellLocalCommand("turns", "turns [N]", "Turn fold view (error turns first)", "查看回合折叠历史"),
    _ShellLocalCommand("view", "view [mode]", "Turn output view (compact/full)", "切换回合输出视图", aliases=("ui",)),
    _ShellLocalCommand("cron", "cron", "Cron setup/query and enable|disable jobs", "管理 cron 与调度开关"),
    _ShellLocalCommand("heart", "heart", "Natural language heartbeat setup/query", "管理心跳状态"),
    _ShellLocalCommand("queue", "queue ...", "Queue actions (status/process/recover/replay)", "查看或处理投递队列"),
    _ShellLocalCommand("locks", "locks", "Show lock status and lease countdown", "查看锁与租约状态"),
    _ShellLocalCommand("ops", "ops [section]", "Ops workbench (runtime/watchdog/lanes/model/mcp/memory/all)", "运行运维面板操作", aliases=("workbench",)),
    _ShellLocalCommand("runtime", "runtime", "Runtime panel (metrics/health/queue/bg)", "查看运行时诊断", aliases=("metrics",)),
    _ShellLocalCommand("watchdog", "watchdog", "Watchdog scans and decision board", "查看 watchdog 扫描与决策"),
    _ShellLocalCommand("lanes", "lanes", "Lane and delivery board", "查看车道与投递面板"),
    _ShellLocalCommand("delivery", "delivery", "Delivery queue panel and actions", "查看或处理投递队列"),
    _ShellLocalCommand("model", "model", "Model/Resilience/Critic panel", "模型/韧性/评审面板"),
    _ShellLocalCommand("critic", "critic", "Critic/refiner panel", "运行评审模式"),
    _ShellLocalCommand("background", "background", "Background task panel and logs", "查看后台任务与日志"),
    _ShellLocalCommand("validate", "validate [run]", "Run unified validation and refresh ops board", "运行统一验证管道"),
    _ShellLocalCommand("context", "context", "Show context compaction snapshot", "查看上下文压缩快照"),
    _ShellLocalCommand("memory", "memory ...", "Memory files + prefs (remember/forget/prefs/audit/timeline/full)", "查看记忆条目与偏好", aliases=("momery",)),
    _ShellLocalCommand("skills", "skills ...", "Skill runtime (list/load/recent)", "查看已安装技能"),
    _ShellLocalCommand("skill", "skill", "Show legacy skill quick help", "查看单个技能帮助"),
    _ShellLocalCommand("retrieval", "retrieval ...", "Retrieval routing (show/decide/run)", "运行检索路由命令"),
    _ShellLocalCommand("compact", "compact [N]", "Compact older context turns (default N=12)", "压缩历史上下文回合"),
    _ShellLocalCommand("mcp", "mcp", "Show MCP server quick help", "查看 MCP 连接器"),
    _ShellLocalCommand("new", "new", "Start a new chat during a conversation", "新建会话", aliases=("reset",)),
    _ShellLocalCommand("clear", "clear", "Clear screen", "清屏", aliases=("cls",)),
    _ShellLocalCommand("welcome", "welcome", "Show welcome panel again", "重新显示欢迎面板", aliases=("banner",)),
    _ShellLocalCommand("exit", "exit", "Exit shell", "退出 shell", aliases=("quit", "q")),
)
...
def _render_custom_cron_message(spec: dict[str, Any]) -> str:
    ...
    if template == "current_time":
        return f"闂傚倸鍊搁崐宄懊归崶褏鏆﹂柣銏㈩焾缁愭鏌熼柇锕€鏋涢柛銊︾箞楠炴牕菐椤掆偓閻忣亝绻涢崨顖毿ｅǎ鍥э躬婵″爼宕ㄩ鍏碱仩缂傚倷鑳舵慨鎶藉础閹惰棄钃熸繛鎴炃氬Σ鍫熸叏濡も偓閻楀﹪寮幆褉鏀介柣鎰级閸ｈ棄鈹戦悙鈺佷壕闂備礁鎼惌澶屽緤閸婄喓浜芥繝鐢靛仜濡瑩宕曢崘娴嬫灁妞ゆ挾濮风壕钘夈€掑顒佹悙濞存粍绮庣槐鎺撳緞婵犲嫮楔閻庢鍠涢褔鍩ユ径鎰潊闁冲搫鍊瑰▍鍥⒒娴ｇ懓顕滅紒璇插€歌灋婵炴垟鎳為崶顒€惟闁冲搫鍊甸幏? {now}"
...
class CodeLiteShell:
    ...
    def _read_live_input(self) -> str:
        ...
        while True:
            ...
            key = msvcrt.getwch()
            if key in {"\r", "\n"}:
                if self._is_live_input_mode_toggle(key, ctrl_pressed=_is_ctrl_pressed()):
                    model.toggle_mode()
            ...
            if key == "\x08":
                ...
                if self._is_live_input_mode_toggle(key, shift_pressed=_is_shift_pressed()):
                    model.toggle_mode()
                    line_count = self._paint_live_input(model, previous_line_count=line_count)
                    continue
                ...
                if self._is_live_input_mode_toggle(key, extended_key=extended):
                    model.toggle_mode()
                    line_count = self._paint_live_input(model, previous_line_count=line_count)
                    continue
                if extended == "K":
                    ...
                    pass
            elif key == "\x10":  # Ctrl+P toggles command focus
                model.toggle_focus()
    ...
    def _run_agent_loop_turn(self, prompt: str, *, require_plan: bool) -> str:
        run_turn = self.services.agent_loop.run_turn
        if not require_plan:
            return run_turn(self.session_id, prompt)
        try:
            parameters = inspect.signature(run_turn).parameters
        except (TypeError, ValueError):
            parameters = {}
        if "require_plan" in parameters:
            return run_turn(self.session_id, prompt, require_plan=True)
        return run_turn(self.session_id, prompt)
    ...
    def _run_agent_turn(self, raw: str) -> None:
        ...
        if self.renderer.is_codex_style() and self._supports_status_block_streaming():
            self._refresh_status_block_live()
    ...
    def _status_display_lines(self) -> list[str]:
        ...
        compact_lines = []
        for item in lines:
            plain = self.renderer._strip_ansi(item).strip()
            if plain.startswith("milestone ") or plain.startswith("next:"):
                continue
            compact_lines.append(item)
        if not compact_lines:
            compact_lines = lines
        for prefix in ("[ERR]", "[DONE]"):
            for item in reversed(compact_lines):
                if self.renderer._strip_ansi(item).strip().startswith(prefix):
                    return [item]
        for item in reversed(compact_lines):
            if not self.renderer._strip_ansi(item).strip().startswith("[TASK]"):
                return [item]
    ...
    def _print_status_block(self) -> None:
        if not self.renderer.is_codex_style():
            return
        if self._supports_status_block_streaming():
            self._refresh_status_block_live()
            return
    ...
    def _describe_runtime_event(self, event: dict[str, Any]) -> tuple[str, str, str] | None:
        ...
        if event_type == "retrieval_decision":
            ...
            enough = decision.get("enough")
            reason = str(decision.get("reason", "")).strip()
            return ("retrieve", f"retrieval route={route} enough={enough} {reason}".strip(), "Retrieve")
        if event_type == "model_request":
            return ("think", f"model request step={payload.get('step', '?')}", "Think")
        if event_type == "model_response":
            tool_call_count = int(payload.get("tool_call_count", 0) or 0)
            return ("think", f"model response tools={tool_call_count}", "Think")
        if event_type == "resilience_result":
            profile = payload.get("preferred_profile") or payload.get("selected_profile") or "default"
            return ("think", f"resilience profile={profile}", "Think")
    ...
    def _split_team_user_request(user_request: str) -> list[str]:
        ...
        for raw_line in user_request.splitlines():
            ...
            if not line:
                continue
            matched = re.match(r"^(?:\d+[.)]|[A-Za-z][.)])\s*(.+)$", line)
            if matched:
    ...
    def _parse_nl_cron_message(self, text: str) -> str:
        lowered = text.lower()
        if any(token in lowered for token in ("current time", "what time", "now", "time now")) or any(
            token in text for token in ("锟斤拷前时锟斤拷", "锟斤拷锟斤拷时锟斤拷", "锟斤拷锟节硷拷锟斤拷")
        ):
            return "current_time"

        quoted = re.search(r'["锟斤拷](.+?)["锟斤拷]', text)
        if quoted:
            return quoted.group(1).strip()

        reminder = re.search(
            r"(?:output|print|send to terminal|remind|锟斤拷锟斤拷|锟斤拷锟絴锟斤拷印)\s+(.+)",
            text,
            re.IGNORECASE,
        )
        if reminder:
    ...
    def _handle_heart_command(self, args: list[str]) -> None:
        ...
        queue_depth = _parse_nl_heart_number(text, r"queue\s*(\d+)", r"闂傚倸鍊搁崐鎼佸磹閹间礁纾归柟闂寸绾惧綊鏌熼梻瀵割槮缁炬儳缍婇弻鐔兼⒒鐎靛壊妲紒鐐劤缂嶅﹪寮婚悢鍏尖拻閻庨潧澹婂Σ顕€姊哄Ч鍥р偓銈夊窗閺嶎厽绠掗梻浣侯焾缁绘劙宕ョ€ｎ剛绀婇柟瀵稿Х绾惧ジ鏌熼柇锕€寮炬繛鍫熺矋椤ㄣ儵鎮欑€电鈪归柤鎸庡姈閵囧嫰骞掗崱妞惧濠电姷顣介埀顒傚仺閸嬨垽鏌＄仦鍓ф创濠碘剝鎮傛俊鐑藉Ψ閵忕姳澹曢梺绉嗗嫷娈旈柦鍐枛閺屻劌鈹戦崱妯侯€涘┑鐐茬毞閺呮粓濡甸崟顖氱闁瑰瓨绺鹃崑鎾诲磹閻曚焦锟?(\d+)", default=0)
        active_task_count = _parse_nl_heart_number(text, r"active\s*(\d+)", r"濠电姷鏁告慨鐑藉极閸涘﹥鍙忛柣鎴ｆ閺嬩線鏌熼梻瀵割槮缁惧墽绮换娑㈠箣濞嗗繒鍔撮梺杞扮椤戝棝濡甸崟顖氱閻犺櫣鍎ら悗楣冩⒑閸涘﹦鎳冪紒缁樺姌閻忓啴姊洪幐搴ｇ畵闁瑰啿閰ｅ鎼佸Χ婢跺鍘告繛杈剧到婢瑰﹪宕曢幋锔界厵闁圭粯甯楅崯鐐烘煙椤栨稒顥堝┑鈩冩倐婵＄柉顦撮柡澶夌矙濮婄粯绗熼埀顒€顭囪婢ф繈姊洪崫鍕櫤缂侇喗鎸搁悾鐑藉箛閺夊潡鏁滃┑掳鍊撻懗鍫曞储?(\d+)", default=0)
        failure_streak = _parse_nl_heart_number(text, r"failure\s*(\d+)", r"濠电姷鏁告慨鐑藉极閸涘﹥鍙忛柣鎴ｆ閺嬩線鏌涘☉姗堟敾闁告瑥绻愰湁闁稿繐鍚嬬紞鎴︽煕閵娿儱鈧湱鎹㈠☉姗嗗晠妞ゆ棁宕甸惄搴ｇ磼閻愵剙鍔ゆ繛纭风節瀵濡堕崥銈呮贡閳ь剨缍嗛崑鍛存偟閺冨牊鈷戦柟鑲╁仜婵＄晫绱掔拠鑼ⅵ鐎殿喛顕ч埥澶愬閻樻鍟嬫繝寰锋澘鈧劙宕戦幘缁樺€垫慨妯煎帶婢у鈧鍠楅幃鍌氱暦閹烘埈娼╅柛蹇曞帶閻︽粓姊绘笟鈧褔鎮ч崱娑樼９闁荤喐澹嬮弸宥夋煕閹伴潧鏋熼柍?(\d+)", default=0)
        latency_ms = _parse_nl_heart_float(text, r"latency\s*(\d+(?:\.\d+)?)", r"闂傚倸鍊搁崐鎼佸磹瀹勬噴褰掑炊椤掑鏅悷婊冪Ч濠€渚€姊虹紒妯虹伇婵☆偄瀚竟鏇熺節濮橆厾鍙嗗┑鐘绘涧濡厼危閸濄儳妫柟顖嗗懐楔闂佸搫澶囬埀顒佸墯閸氬骞栫划鍏夊亾瀹曞浂鍟囩紓鍌欒兌閸嬫捇宕曢崘宸劷闁跨喓濮撮拑鐔兼煏閸繃鍋柛锔诲幐閸嬫捇鏁愭惔鈥茬凹闂佺粯绻冨ú鐔煎箖濡ゅ啯鍠嗛柛鏇ㄥ墰椤︺儵鎮楀▓鍨灈濠⒀冮叄楠炴垿濮€閻橆偅顫嶉梺闈涚箚閸撴繂鈻嶅鍫熲拺闁硅偐鍋涢崝妤呮煛閸涱喚鐭掗柟?(\d+(?:\.\d+)?)", default=0.0)
        error_match = re.search(r"(?:error|闂傚倸鍊搁崐鎼佸磹閹间礁纾归柟闂寸绾惧綊鏌ｉ幋锝呅撻柛濠傛健閺屻劑寮崼鐔告闂佺顑嗛幐鍓у垝椤撶偐妲堟俊顖濐嚙椤︹晠姊虹粙娆惧劀缂佺粯绻堝濠氬即閻旇櫣顔曢梺缁樺姦閸撴岸鎮甸敃鍌涒拺?\s*[:闂傚倸鍊搁崐鎼佸磹閹间礁纾归柣鎴ｅГ閸婂潡鏌ㄩ弮鍫熸殰闁稿鎸剧划顓炩槈濡娅ч梺娲诲幗閻熲晠寮婚悢鍏煎€绘慨妤€妫欓悾鐑芥⒑?\s*(.+)$", text, re.IGNORECASE)
